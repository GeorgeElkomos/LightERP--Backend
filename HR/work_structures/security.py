...
def validate_location_scope(user, enterprise_id=None, business_group_id=None):
    """
    Validate that the user has permission to CRUD a location in the given 
    Enterprise and/or Business Group.
    
    Access Rules:
    1. If business_group_id is provided: User must have access to that BG (or a dept in it).
    2. If only enterprise_id is provided: User must have access to at least one BG 
       within that Enterprise.
    
    Args:
        user: The user performing the action
        enterprise_id: Enterprise ID for the location
    ...
    if enterprise_id:
        # Check if user has access to any Business Group within this Enterprise        # User's accessible BG IDs
        user_bg_ids = UserDataScope.objects.filter(user=user).values_list('business_group_id', flat=True)
        
        # Check if any of these BGs belong to the target Enterprise
        if BusinessGroup.objects.filter(enterprise_id=enterprise_id, id__in=user_bg_ids).exists():
            return
            
        raise PermissionDenied("You do not have access to this Enterprise")
        
    raise PermissionDenied("Location must be linked to either an Enterprise or a Business Group")
