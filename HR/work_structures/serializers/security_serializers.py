            ...
            for bg_value in business_groups:
                ...
                try:
                    if str(bg_value).isdigit():
                        bg = BusinessGroup.objects.filter(id=int(bg_value)).active().first()
                        if not bg:
                            raise BusinessGroup.DoesNotExist
                    else:
                        bg = BusinessGroup.objects.filter(code=bg_value).active().first()
                        if not bg:
                            raise BusinessGroup.DoesNotExist
            ...
            for dept_value in departments:
                ...
                try:
                    if str(dept_value).isdigit():
                        dept = Department.objects.active().get(id=int(dept_value))
                    else:
                        dept = Department.objects.active().get(code=dept_value)
