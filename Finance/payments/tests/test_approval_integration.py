...
from Finance.GL.models import JournalEntry, JournalLine, XX_Segment_combination
...
class PaymentApprovalIntegrationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.manager_role, _ = JobRole.objects.get_or_create(
            name="manager",
            defaults={'code': 'MANAGER_ROLE_PAY_INT'}
        )
        self.director_role, _ = JobRole.objects.get_or_create(
            name="director",
            defaults={'code': 'DIRECTOR_ROLE_PAY_INT'}
        )
        ...
        # Create payment GL with lines matching the allocation amount (500.00)
        self.payment_gl = self._create_gl_entry_with_lines(
            currency=self.currency, amount=Decimal("500.00"), memo="Test Payment GL"
        )
    ...
    def _create_user(self, email, name, phone_number, role=None):
        ...
        if role:
            ...
            # Attach role to user instance for convenience in tests
            user.job_role = role
    ...
    def _create_gl_entry_with_lines(self, currency, amount, memo="Test GL"):
        """
        Helper to create a properly balanced GL entry with journal lines.

        Creates a GL entry with one debit line and one credit line for the specified amount.
        """
        # Create a simple segment combination for testing
        segment_combo = XX_Segment_combination.objects.create(
            description="Test segment combination"
        )

        # Create the GL entry
        gl_entry = JournalEntry.objects.create(
            date=date.today(), currency=currency, memo=memo
        )
        ...
        JournalLine.objects.create(
            ...
            amount=amount,
            type="DEBIT",
            segment_combination=segment_combo,
        )

        # Create credit line (for payment, credit represents the payment amount)
        JournalLine.objects.create(
            entry=gl_entry,
            amount=amount,
            type="CREDIT",
    ...
    def test_manager_can_approve_first_stage(self):
        ...
        workflow = self.payment.approve_by_user(
            self.manager, comment="Approved by manager"
        )
