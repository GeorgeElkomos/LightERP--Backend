...
class ServicePRCreateTests(TestCase):
    ...
    def test_create_service_pr_success(self):
        ...
        self.assertTrue(response.data['pr_number'].startswith('PR-SRV'))
        self.assertEqual(response.data['requester_name'], 'Bob Johnson')
        self.assertEqual(response.data['status'], 'DRAFT')
        
        # Verify PR was created
        pr = Service_PR.objects.get(pr_id=response.data['pr_id'])
