...
class NonCatalogPRCreateTests(TestCase):
    ...
    def test_create_noncatalog_pr_success(self):
        ...
        self.assertTrue(response.data['data']['pr_number'].startswith('PR-NC'))
        self.assertEqual(response.data['data']['requester_name'], 'Jane Smith')
        self.assertEqual(response.data['data']['status'], 'DRAFT')
        
        # Verify PR was created
        pr = NonCatalog_PR.objects.get(pr_id=response.data['data']['pr_id'])
