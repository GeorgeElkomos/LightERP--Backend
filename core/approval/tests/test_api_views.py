...
class WorkflowTemplateAPITest(TestCase):
    ...
    def test_filter_workflow_templates_by_active(self):
        ...
        self.assertEqual(response.data[0]['code'], 'TEST_INVOICE_APPROVAL')
        
        # Filter inactive
        response = self.client.get(url, {'is_active': 'false'})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['code'], 'TEST_PO_APPROVAL')
    ...
    def test_filter_workflow_templates_by_code(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['name'], 'Test Invoice Approval')
    ...
    def test_get_workflow_template_detail(self):
        ...
        url = reverse('core:approval:workflow-template-detail', args=[self.template1.id])
        response = self.client.get(url)
    ...
    def test_delete_workflow_template(self):
        ...
        url = reverse('core:approval:workflow-template-detail', args=[self.template2.id])
    ...
    def test_get_workflow_template_stages(self):
        ...
        url = reverse('core:approval:workflow-template-stages', args=[self.template1.id])
        response = self.client.get(url)
        
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)
        self.assertEqual(response.data[0]['order_index'], 1)
        self.assertEqual(response.data[1]['order_index'], 2)
...
class StageTemplateAPITest(TestCase):
    ...
    def test_filter_stage_templates_by_workflow(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)
        self.assertEqual(response.data[0]['name'], 'Stage 1')
    ...
    def test_update_stage_template_order_conflict(self):
        ...
        url = reverse('core:approval:stage-template-detail', args=[self.stage1.id])
        data = {
            'order_index': 2  # Already used by stage2
        }
...
class UtilityViewsAPITest(TestCase):
    ...
    def test_list_content_types_only_linked_modules(self):
        ...
        returned_ids = [ct['id'] for ct in response.data]
        self.assertIn(self.invoice_ct.id, returned_ids)
        self.assertIn(po_ct.id, returned_ids)
    
    def test_get_workflow_instance_detail(self):
        """Test GET /workflow-instances/{id}/"""
        # Create invoice and workflow
        invoice = TestInvoice.objects.create(
            invoice_number='INV-UTIL-001',
            vendor_name='Test Vendor',
            total_amount=1000.00,
            description='Test'
        )
        ...
        url = reverse('core:approval:workflow-instance-detail', args=[workflow.id])
        response = self.client.get(url)
        
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['object_id'], invoice.id)
        self.assertIn('stage_instances', response.data)
    
    def test_get_workflow_instances_by_object(self):
        """Test GET /workflow-instances/by-object/"""
        # Create invoice and workflow
        invoice = TestInvoice.objects.create(
            invoice_number='INV-UTIL-002',
            vendor_name='Test Vendor',
        ...
        url = reverse('core:approval:workflow-instances-by-object')
        response = self.client.get(url, {
            'content_type': self.invoice_ct.id,
    ...
    def test_get_workflow_instances_by_object_missing_params(self):
